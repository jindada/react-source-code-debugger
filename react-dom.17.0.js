...
(function (global, factory) {
  ...
  function createInstance(
    ...
    hostContext,
    internalInstanceHandle // 当前的fiber对象 workingInProgress
  ) {
    var parentNamespace;
  ...
  /**
   * TODO 深入了解
   * mount阶段 shouldTrackSideEffects = false
   * update阶段 shouldTrackSideEffects = true
   */
  function ChildReconciler(shouldTrackSideEffects) {
    ...
    /**
     * @params
     * returnFiber 父级fiber 也就是workingInProgress fiber
     * currentFirstChild current fiber的第一个子节点
     * newChild 新的children对象
     */
  ...
  function updateFunctionComponent(
  ...
  ) {
    {
      ...
      // fiber.type: 一般来讲和fiber.elementType一致.
  ...
  function updateClassComponent(
    ...
    // 与fiber关联的局部状态节点
    // 比如:
    //    HostComponent类型指向与fiber节点对应的 dom 节点;
    ...
    if (instance === null) {
      ...
      // current === null 代表了初始化阶段
      // 初始化阶段 但是 instance !== null 的情况出现在 多次挂载一个类组件
  ...
  function updateHostComponent(current, workInProgress, renderLanes) {
    ...
    // 2. 获取下级`ReactElement`对象
    var nextChildren = nextProps.children;
    // 是否只有一个单独的文本子节点 单独文本子节点没有对应的fiber节点(优化)
  ...
  function scheduleUpdateOnFiber(fiber, lane, eventTime) {
    ...
    // TODO 获取更新lane从FiberToRoot
    // markUpdateLaneFromFiberToRoot(fiber, lane)函数在fiber树构造(对比更新)中才会发挥作用,
  ...
  /**
   * performSyncWorkOnRoot的逻辑, 分为 3 部分:
   *  1 fiber 树构造
  ...
  function performSyncWorkOnRoot(root) {
    ...
    // finishedWork 本次render生成的最终分fiber树
    ...
    // TODO 再次调用 处理可能存在的更新 例如componentDidMount中调用setState
    ensureRootIsScheduled(root, now());
    return null;
  }
  ...
  function renderRootSync(root, lanes) {
    ...
    if (
      ...
      /**
       * 在React源码中, 每一次执行fiber树构造(也就是调用performSyncWorkOnRoot或者performConcurrentWorkOnRoot函数)的过程,
       * 都需要一些全局变量来保存状态. 在之前已经介绍最核心的全局变量.
       * 如果从单个变量来看, 它们就是一个个的全局变量.
       * 如果将这些全局变量组合起来, 它们代表了当前fiber树构造的活动记录.
       * 通过这一组全局变量, 可以还原fiber树构造过程(比如时间切片的实现过程(参考React 调度原理), fiber树构造过程被打断之后需要还原进度, 全靠这一组全局变量).
       * 所以每次fiber树构造是一个独立的过程, 需要独立的一组全局变量,
       * 在React内部把这一个独立的过程封装为一个栈帧stack
       */
      // 刷新栈帧, legacy模式下都会进入
      // TODO 赋值workInProgress workInProgressRoot
      // debugger;
  ...
  function performUnitOfWork(unitOfWork) {
    ...
    // console.log("beginWork开始", unitOfWork);
    // debugger;

    if ((unitOfWork.mode & ProfileMode) !== NoMode) {
      startProfilerTimer(unitOfWork);
      // 感觉(current)WorkInProgess.alternate 初始化也应该是null啊???
      // unitOfWork = WorkInProgess
    ...
    // console.log("beginWork结束", unitOfWork);
    // debugger;
    ...
    if (next === null) {
      ...
      console.log("completeWork开始", unitOfWork);
      // debugger;
      completeUnitOfWork(unitOfWork);
      console.log("completeWork结束", unitOfWork);
      // debugger;
    } else {
      // 否则workInProgress = 派生出来的新节点 循环继续
      workInProgress = next;
    }
  ...
  function commitRoot(root) {
    ...
    /**
     * runWithPriority$1 由调度器scheduler提供
     * 第一个参数 优先级
     * 第二个参数 回调函数
     */
    runWithPriority$1(
    ...
    while (nextEffect !== null) {
      ...
      if ((flags & Passive) !== NoFlags) {
        ...
        if (!rootDoesHavePassiveEffects) {
          ...
          /**
           * 先用NormalPriority$1的优先级注册flushPassiveEffects
           */
  ...
  /**
   * 参数root.current即HostRootFiber,
   * 作用是给HostRootFiber创建一个alternate副本.workInProgress指针指向这个副本(即workInProgress = HostRootFiber.alternate),
   * HostRootFiber.alternate是正在构造的fiber树的根节点.
   */
  function createWorkInProgress(current, pendingProps) {
    var workInProgress = current.alternate;

    /**
     * 初始化阶段 workInProgress === null
     * 注意其中的createWorkInProgress(root.current, null),
     *
     */
    if (workInProgress === null) {
      ...
      // workInProgress current 互相用alternate关联上
  ...
  function createHostRootFiber(tag) {
    var mode;
    // 创建HostRootFiber时, 其中fiber.mode属性, 会与 3 种RootTag(ConcurrentRoot,BlockingRoot,LegacyRoot)关联起来.
    // fiber树中所有节点的mode都会和HostRootFiber.mode一致(新建的 fiber 节点, 其 mode 来源于父节点),
